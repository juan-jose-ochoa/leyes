#!/bin/bash
#
# LeyesMX - Script de inicio de servicios (Astro)
#
# Uso:
#   ./start-astro.sh          # Inicia backend (PostgREST) y frontend Astro
#   ./start-astro.sh backend  # Solo backend
#   ./start-astro.sh frontend # Solo frontend
#   ./start-astro.sh db       # Solo verificar base de datos
#
...
start_frontend() {
    log_info "Iniciando frontend Astro en puerto $ASTRO_PORT..."
    ...
    # Verificar si ya esta corriendo
    if lsof -i :$ASTRO_PORT &> /dev/null; then
        log_warn "Puerto $ASTRO_PORT ya en uso"
        cd ..
        return 0
    fi
    ...
    if kill -0 $FRONTEND_PID 2>/dev/null; then
        log_success "Frontend Astro iniciado (PID: $FRONTEND_PID)"
        log_info "  App disponible en: http://localhost:$ASTRO_PORT"
        echo $FRONTEND_PID > ../.frontend-astro.pid
    else
        log_error "Frontend Astro fallo al iniciar"
        cd ..
        return 1
    fi
...
stop_services() {
    ...
    if [ -f .frontend-astro.pid ]; then
        PID=$(cat .frontend-astro.pid)
        if kill -0 $PID 2>/dev/null; then
            kill $PID
            log_success "Frontend Astro detenido"
        fi
        rm .frontend-astro.pid
    fi
...
# Mostrar ayuda
show_help() {
    echo "LeyesMX - Script de inicio (Astro)"
    echo ""
    echo "Uso: $0 [comando]"
    echo ""
    echo "Comandos:"
    echo "  (sin args)  Inicia backend y frontend Astro"
    echo "  backend     Solo inicia PostgREST API"
    echo "  frontend    Solo inicia servidor de desarrollo Astro"
    ...
    echo "  Astro:      $ASTRO_PORT"
}

# Mostrar estado
show_status() {
    echo "=== Estado de Servicios (Astro) ==="
    echo ""
    ...
    # Frontend Astro
    if lsof -i :$ASTRO_PORT &> /dev/null; then
        echo -e "Astro:       ${GREEN}ACTIVO${NC} (http://localhost:$ASTRO_PORT)"
    else
        echo -e "Astro:       ${RED}INACTIVO${NC}"
    fi
}
...
main() {
    echo ""
    echo "=========================================="
    echo "     LeyesMX - Servidor Local (Astro)"
    echo "=========================================="
    echo ""
