...
Uso:
    python scripts/leyesmx/checksums.py CFF --guardar    # Guarda checksums actuales como referencia
    python scripts/leyesmx/checksums.py CFF --comparar  # Compara BD contra referencia guardada
    python scripts/leyesmx/checksums.py CFF --diff 66   # Muestra diferencia de un artículo específico
"""
...
def comparar_checksums(ley: str) -> dict:
    ...
    if not ruta.exists():
        ...
        print(f"Ejecuta primero: python scripts/leyesmx/checksums.py {ley} --guardar")
        return None
...
def main():
    ...
    if accion == '--guardar':
        ...
        if total == 0:
            ...
            print(f"\nTotal: {total} cambios")
            print(f"\nPara ver contenido: python scripts/leyesmx/checksums.py {ley} --diff <numero>")
            print(f"Para aceptar cambios: python scripts/leyesmx/checksums.py {ley} --guardar")
