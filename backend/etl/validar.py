...
Uso:
    ...
    python backend/etl/validar.py CFF --detalle
"""
    ...
    def cargar_archivos(self) -> bool:
        ...
        if not self.cargar_estructura_bd():
            ...
            if not self.cargar_estructura_archivo():
                ...
                print("       Ejecuta: python backend/etl/extraer_mapa.py", self.codigo)
                return False

        if not self.contenido_path.exists():
            print(f"ERROR: {self.contenido_path.name} no existe")
            print("       Ejecuta primero: python backend/etl/extraer.py", self.codigo)
            return False
...
def main():
    if len(sys.argv) < 2:
        print("Uso: python backend/etl/validar.py <CODIGO> [--detalle]")
        sys.exit(1)
